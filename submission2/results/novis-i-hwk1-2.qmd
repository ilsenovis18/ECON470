...
I remove the relevant plans just by applying the relevant filter to the full MA data and then creating the table of plan types. Counts of different plan types with these exclusions are presented in @plans_per_type_updated.

```{python}

#| echo: false
#| label: plans_per_type_updated
...
plans_per_type_updated = final_ma_data.pivot_table(index='plan_type', columns='year', values='planid', aggfunc='count')
plans_per_type_updated.index.name = 'Plan Type'
plans_per_type_updated = plans_per_type.sort_values(by=2015, ascending=False)

display(Markdown(plans_per_type_updated.to_markdown()))
...
To do this, I need to collapse the data to the plan-type level and form the average enrollments across all counties in which that plan type operates. The resulting table is presented in @enrollment_per_type.
